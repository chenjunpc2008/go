...
func svrNewConnection(conn net.Conn, clientID uint64, cliIP string, cliAddr string, svr *CtcpsvrSt) *clientSessnSt {
	cliSessn := svr.cliSns.addNewConnection(conn, clientID, cliIP, cliAddr)
	svr.handler.OnNewConnection(clientID, cliIP, cliAddr)
	return cliSessn
}

// report to server disconnect from one client
func svrDisconnect(clientID uint64, cliIP string, cliAddr string, svr *CtcpsvrSt) {
...
func cliDataRcved(clientID uint64, cliIP string, cliAddr string, length int, rawData []byte,
	svr *CtcpsvrSt, asyncReceive bool) []byte {
